    ...
    onMessage(data) {
        ...
        if (distance === 0 && currentWord !== '') {
            ...
            if (games[socket.roomID].drawer !== socket.id && !socket.hasGuessed) {
                ...
                socket.emit('correctGuess', { message: 'پێشبینییەکەت ڕاست بوو!', id: socket.id });
                socket.broadcast.emit('correctGuess', { message: `${socket.player.name} وشەکەی دۆزییەوە!`, id: socket.id });
            ...
            if (games[socket.roomID].drawer !== socket.id && !socket.hasGuessed) socket.emit('closeGuess', { message: 'زۆر لە وشەکەوە نزیکیت!' });
        } else {
            io.in(socket.roomID).emit('message', { ...data, name: socket.player.name });
        }
    }
